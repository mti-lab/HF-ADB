...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--instance_data_dir_for_mask",
        type=str,
        default=None,
        required=True,
        help="A folder containing the mask of instance images.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.05,
        help="The noise budget for pgd outside mask.",
    )
    parser.add_argument(
        "--mask_pgd_eps",
        type=float,
        default=0.05,
        help="The noise budget for pgd inside mask.",
    )
    parser.add_argument(
        "--target_image_path",
        default=None,
...
def load_mask(mask_dir, size=512, center_crop=True) -> torch.Tensor:
    image_transforms = transforms.Compose(
        [
            transforms.Resize(size, interpolation=transforms.InterpolationMode.BILINEAR),
            transforms.CenterCrop(size) if center_crop else transforms.RandomCrop(size),
    ...
    images = [image_transforms(ImageOps.invert(Image.open(i).convert("RGB"))) for i in sorted(list(Path(mask_dir).iterdir()))]
    images = torch.stack(images)
    return images
...
def pgd_attack(
    ...
    for step in range(num_steps):
        ...
        eps = args.pgd_eps
        mask_eps = args.mask_pgd_eps

        adv_images = perturbed_images + alpha * perturbed_images.grad.sign()
        eta_in_mask = torch.clamp(adv_images - original_images, min=-mask_eps, max=+mask_eps) * mask_tensor
        eta_out_mask = torch.clamp(adv_images - original_images, min=-eps, max=+eps) * (1 - mask_tensor)
        perturbed_images = torch.clamp(original_images + eta_in_mask + eta_out_mask, min=-1, max=+1).detach_()
...
def main(args):
    ...
    mask_data = load_mask(
        args.instance_data_dir_for_mask,
        size=args.resolution,
        center_crop=args.center_crop
    )
    ...
    for i in range(args.max_train_steps):
        ...
        if (i + 1) % args.checkpointing_iterations == 0:
            ...
            img_names = [
                ...
                for instance_path in sorted(list(Path(args.instance_data_dir_for_adversarial).iterdir()))
            ]
            for img_pixel, img_name in zip(noised_imgs, img_names):
                save_file_name = "".join(img_name.split('.')[:-1]) + ".png"
                save_path = os.path.join(save_folder, f"{i+1}_noise_{save_file_name}")
                Image.fromarray(
